...
public interface LedgerEntryRepository extends JpaRepository<LedgerEntry, Long> {
    ...
    // @Query("SELECT new com.room7.moneygement.dto.LedgerEntryDTO(e.date, SUM(e.amount), MIN(e.description), MIN(e.category.categoryName), e.ledgerType) FROM LedgerEntry e WHERE e.ledgerType = false AND MONTH(e.date) = :month AND YEAR(e.date) = :year GROUP BY e.date ORDER BY e.date")
    // List<LedgerEntryDTO> findIncomeSummaryByMonthAndYear(@Param("year") int year, @Param("month") int month);
